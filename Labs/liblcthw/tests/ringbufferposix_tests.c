...
static RingBufferPOSIX *buffer = NULL;
...
{
    debug("----- test_create");
    buffer = RingBufferPOSIX_create(15);
    mu_assert(buffer != NULL, "Failed to create ring buffer.");
    mu_assert(buffer->read == buffer->write, "Buffer should start empty.");
...
char *test_read_write()
{
    ...
    // Test writing
    int rc = RingBufferPOSIX_write(buffer, test_data, TEST_DATA_LEN);
    mu_assert(rc == TEST_DATA_LEN, "Failed to write full data.");
    mu_assert(!(buffer->write == buffer->read), "Buffer should not be empty after write.");
    mu_assert(RingBufferPOSIX_available_data(buffer) == TEST_DATA_LEN, 
             "Available data should match written length.");
    
    // Test reading
    char target[TEST_DATA_LEN];
    rc = RingBufferPOSIX_read(buffer, target, TEST_DATA_LEN);
    ...
    mu_assert(buffer->write == buffer->read, "Buffer should be empty after read.");
